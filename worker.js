    ...
    async function createBuyOfferFromBuyAmountReply({ link, chat, from, text, req, messageId, replyToMessageId }) {
      ...
      if (reqRec?.mwTgId) {
        ...
        offer.user = {
          ...
          mwTgId: reqRec.mwTgId,
          mwReqId: reqRec.mwReqId || "",
          mwWebhookBase: reqRec.mwWebhookBase || "https://client-qr-pay.kireeshka73.workers.dev",
        };
      }
    ...
    async function setBuyOfferStatusFromChat({ offerId, nextStatus, actor, req }) {
      ...
      if (!offer.moonWalletUser && !offer.user?.mwTgId) {
        try {
          ...
          if (reserve?.moonWalletUser?.tgId) {
            ...
            offer.user.mwWebhookBase = reserve.moonWalletUser.mwWebhookBase || "https://client-qr-pay.kireeshka73.workers.dev";
            ...
            if (reqRec?.mwTgId) {
              ...
              offer.user.mwWebhookBase = reqRec.mwWebhookBase || "https://client-qr-pay.kireeshka73.workers.dev";
            }
          }
        } catch {}
      }
      ...
      try {
        if (status === "SUCCESS" && offer.qrOrder) {
          ...
          const mwWebhookBaseDeal = String(offer.user?.mwWebhookBase || offer.moonWalletUser?.mwWebhookBase || "https://client-qr-pay.kireeshka73.workers.dev").trim();
          if (mwTgIdDeal) {
            ctx.waitUntil(
...
\u041A\u0443\u0440\u0441: ${String(created.offer.rate)}`,
        ...
        if (tgId && username) {
          ...
          if (isPrivateChat && t.startsWith("/start")) {
            const START_IMAGE = "https://postimg.cc/G4pFyTGM";
            ...
            const kb = {
              inline_keyboard: [
                [{ text: "\u0421\u0430\u0439\u0442 \u043E\u0431\u043C\u0435\u043D\u043D\u0438\u043A\u0430", url: "https://crossflag.org" }],
                [
                  { text: "\u041C\u0438\u043D\u0438-\u043F\u0440\u0438\u043B\u043E\u0436\u0435\u043D\u0438\u0435", web_app: { url: "https://crossflag.org" } },
                  { text: "\u041F\u043E\u0434\u0434\u0435\u0440\u0436\u043A\u0430", url: "https://t.me/crossflag2" }
                ]
              ]
            };
...
\u041D\u043E\u0432\u044B\u0439 \u0431\u0430\u043B\u0430\u043D\u0441: ${fmtUsdtE6(next)}.`
    ...
    if (url.pathname === "/api/public/mw_buy_request" && request.method === "POST") {
      try {
        ...
        const mwWebhookBase = "https://client-qr-pay.kireeshka73.workers.dev";
  ...
  return json({
    ...
    if (url.pathname === "/api/admin/broadcast" && request.method === "POST") {
      ...
      if (ctype.includes("multipart/form-data")) {
        ...
        testOnly = fd.get("testOnly") === "true" || fd.get("testOnly") === "1";
        ...
        testOnly = !!bd.testOnly;
      }
      if (!broadcastText) return bad("Missing text", 400);
      const TEST_USERNAME = "xasxca96";
      if (testOnly) {
        try {
          const testMap = await readJsonKV(userKeyByUsername(TEST_USERNAME), null);
          const testUserId = testMap && testMap.userId ? String(testMap.userId) : "";
          const testUser = testUserId ? await readJsonKV(userKeyById(testUserId), null) : null;
          const testChatId = testUser ? String(testUser.chatId || testUser.id || "") : "";
          if (!testChatId) return bad("Test user @" + TEST_USERNAME + " not found or no chatId", 404);
          ...
          return json({ ok: true, testOnly: true, sent: 1 });
        } catch (e) {
          return bad("Test send failed: " + (e.message || String(e)), 500);
        }
      }
      const idx = await readJsonKV(userIndexKey, []);
      const ids = Array.isArray(idx) ? idx.slice(0, 5e4) : [];
